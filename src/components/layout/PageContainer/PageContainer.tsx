...
export const PageContainer = memo(
	({ children, className, maxWidth = '7xl' }: PageContainerProps) => {
		...
		return (
			<div className="min-h-screen md:min-h-full relative overflow-hidden bg-white dark:bg-slate-900 transition-colors duration-300">
				{/* Global Organic Background Blobs */}
				<div className="absolute inset-0 overflow-hidden pointer-events-none">
					<div
						className={cn(
							'absolute top-[-10%] left-[-10%] w-[50vw] h-[50vw] rounded-full mix-blend-multiply dark:mix-blend-screen filter blur-[120px] opacity-50 animate-blob transition-colors duration-1000',
							blobColors[0]
						)}
					/>
					<div
						className={cn(
							'absolute top-[-10%] right-[-10%] w-[50vw] h-[50vw] rounded-full mix-blend-multiply dark:mix-blend-screen filter blur-[120px] opacity-50 animate-blob animation-delay-2000 transition-colors duration-1000',
							blobColors[1]
						)}
					/>
					<div
						className={cn(
							'absolute bottom-[-20%] left-[20%] w-[60vw] h-[60vw] rounded-full mix-blend-multiply dark:mix-blend-screen filter blur-[120px] opacity-50 animate-blob animation-delay-4000 transition-colors duration-1000',
							blobColors[2]
						)}
					/>
				</div>
