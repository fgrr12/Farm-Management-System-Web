...
export function VoiceRecorder({
	...
	const getRecordingButtonText = () => {
		...
		if (isExecuting) return t('voice.executing')
	...
	return (
		<div className={`card bg-base-100 shadow-xl ${className}`}>
			<div className="card-body">
					...
					<div className="flex items-center gap-4">
						...
						{processingResponse && !autoExecute && hasOperationsToExecute && (
							...
							>
								...
								{isExecuting ? t('voice.executing') : t('voice.execute')}
							</button>
						)}
					</div>
					...
					{(isProcessing || isExecuting) && (
						<div className="flex items-center gap-2">
							...
							<span className="text-sm">
								{isProcessing && t('voice.processingAudio')}
								{isExecuting && t('voice.executingOperations')}
							</span>
						</div>
					)}
				</div>
				...
				{audioURL && (
					<div className="mt-4">
						<div className="text-sm font-medium mb-2">{t('voice.recordedAudio')}</div>
				...
				{transcription && (
					<div className="mt-4">
						<div className="text-sm font-semibold mb-2">{t('voice.transcription')}</div>
				...
				{processingResponse && (
					<div className="mt-4">
						<div className="text-sm font-semibold mb-2">{t('voice.aiAnalysis')}</div>
						...
						>
							...
							<span>
								{processingResponse.success
									? t('voice.analysisSuccessful')
									: t('voice.analysisWarning')}
							</span>
						</div>
								...
								{Object.entries(processingResponse.data).map(([type, operations]) => {
									...
									return (
										<div key={type} className="badge badge-primary gap-2">
											...
											{t(`voice.operations.${type}`)}: {operations.length}
										</div>
									)
								})}
							</div>
						...
						{processingResponse.warnings && processingResponse.warnings.length > 0 && (
							<div className="collapse collapse-arrow bg-warning/10 mb-2">
								...
								<div className="collapse-title text-sm font-medium">
									...
									{t('voice.warnings')} ({processingResponse.warnings.length})
								</div>
						...
						{processingResponse.errors && processingResponse.errors.length > 0 && (
							<div className="collapse collapse-arrow bg-error/10 mb-2">
								...
								<div className="collapse-title text-sm font-medium">
									...
									{t('voice.errors')} ({processingResponse.errors.length})
								</div>
								<div className="collapse-content">
									<ul className="list-disc list-inside text-sm space-y-1">
						...
						<div className="flex justify-between text-xs text-base-content/60">
							{processingResponse.processingTime && (
								<span>
									{t('voice.processingTime')}: {processingResponse.processingTime}ms
								</span>
							)}
							{processingResponse.tokensUsed && (
								<span>
									{t('voice.tokensUsed')}: {processingResponse.tokensUsed}
								</span>
							)}
						</div>
					</div>
				...
				{executionResults.length > 0 && (
					<div className="mt-4">
						<div className="text-sm font-semibold mb-2">{t('voice.executionResults')}</div>
								...
								>
									...
									<div className="flex-1">
										<div className="font-medium">
											{t(`voice.operations.${result.type}`)} - {result.operation}
										</div>
